...
def binary_image(regions,regions_updated,canal):
    ...
    def count_and_merge_regions_diff_color(seuil_diff_region=15):
        ...
        print("liste_moyennedans count and merge",liste_moyenne)
            ...
            for j in range (len(liste_moyenne)):
                if i!=j:
                    if np.abs(liste_moyenne[i][0][0]-liste_moyenne[j][0][0])<seuil_diff_region and np.abs(liste_moyenne[i][0][1]-liste_moyenne[j][0][1])<seuil_diff_region and np.abs(liste_moyenne[i][0][2]-liste_moyenne[j][0][2])<seuil_diff_region:
                        print(i)
                        print(liste_moyenne[i])
                        print(j)
                        print(liste_moyenne[j])
                        print("region supprimé par couleur")
    ...
    while len(liste_moyenne)>2 and seuil<=150:
        print("liste_moyenne, Avant SRM",liste_moyenne)
        regions,regions_updated=SRM_segmentation_finish(regions,regions_updated,canal,50)
        liste_moyenne=count_and_merge_regions_diff_color(15)
        image_current=creer_image_regions_3canaux(regions,regions_updated)
        liste_images.append(image_current)
        print("liste_moyenne, debut boucle while",liste_moyenne)
        ...
        #on cherche toutes les regions qui ont plus de 500 pixels
        liste_index=[]
...
def dilatation(elstruc,image):
    image_dilatation=morpho.binary_dilation(image,elstruc)
    return image_dilatation

def masque_binaire_convex_hull(regions,regions_updated):
    liste_des_points=[]
    coord_region,occurence_region,max_occurence_index=pre.detection_plus_grosse_region(regions,regions_updated)
    #retirer la plus grosse région
    coord_region.pop(max_occurence_index)
    occurence_region.pop(max_occurence_index)
    #2ème plus grosse région
    max_occurence_index=np.argmax(occurence_region)
    coord_region_max=coord_region[max_occurence_index]
    for x in range(len(regions_updated)):
        for y in range(len(regions_updated[0])):
            if regions_updated[x][y]==coord_region[max_occurence_index]:
                liste_des_points.append((x,y))
    # Create a convex hull
    hull = ConvexHull(liste_des_points)
    # Create a binary mask with the same shape as regions_updated
    mask = np.zeros((len(regions_updated),len(regions_updated[0])), dtype=bool)
    print(mask.shape)
    
    # Set all points inside the convex hull to True
    for simplex in hull.simplices:
        mask[simplex] = True
    
    return hull, mask

def masque_binaire_convex_hull2(image_binaire):
    liste_des_points=[]
    for x in range(image_binaire.shape[0]):
        for y in range(image_binaire.shape[1]):
            if image_binaire[x][y]==True:
                liste_des_points.append((x,y))
    # Create a convex hull
    hull = ConvexHull(liste_des_points)
    # Create a binary mask with the same shape as regions_updated
    mask = np.zeros((image_binaire.shape), dtype=bool)
    # Set all points inside the convex hull to True
